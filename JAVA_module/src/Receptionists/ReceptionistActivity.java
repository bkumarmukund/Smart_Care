...
public class ReceptionistActivity extends javax.swing.JFrame {
    ...
    private void jLabel7MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel7MouseClicked
        ...
        String newID;
        int idValue;
        //String sql = "SELECT patientID from patient ORDER BY patientID DESC LIMIT 1";
        String sql = "SELECT MAX(CAST(SUBSTRING(patientID, 8, length(patientID)-7) AS UNSIGNED)) AS patientID FROM patient";
        try {
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
            rs.first();
            newPatient np = new newPatient();
            newID = (rs.getString("patientID")).replaceAll("[^0-9]", "");
            idValue = Integer.parseInt(newID);
            idValue = idValue +1;
            np.newPatientID = "smartCP"+idValue;
            np.setVisible(true);
            dispose();

        } catch (SQLException e) {
            System.out.println("New Patient entry block");
        }
